...
Task Build -Depends Test {
    ...
    try {
        ...
        $UMNFileSystemInfo = $DevOpsPackages | Where-Object -FilterScript { $_.normalizedName -eq "UMN-FileSystem" }

        $UMNFileSystemPkg = Invoke-RestMethod -Headers $Headers -Method Get -Uri "https://feeds.dev.azure.com/umn-microsoft/_apis/packaging/Feeds/UMN-Internal/packages/$($UMNFileSystemInfo.id)?api-version=5.0-preview.1"

        [System.Version]$AzureDevOpsVersion = [System.Version]$UMNFileSystemPkg.value.versions.version
